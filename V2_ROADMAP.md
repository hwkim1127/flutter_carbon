...
### Phase 1 — Theming rearchitecture (v2.0, breaking)

1. `CarbonThemeData` drops `ThemeExtension`; add `CarbonTheme` inherited widget
   and re-point `context.carbon`.
2. Add `CarbonApp` (on `WidgetsApp`) with route/navigation parity.
3. Move `carbonTheme()` + all `*ThemeData` Material mappings into the bridge
   (`flutter_carbon_material` package, or `flutter_carbon/material.dart`
   library file — decide before release; a library file is less publishing
   overhead, a package keeps the core's dependency tree honest).
4. Migration guide in `MIGRATION.md`:
   - Material app users: wrap with `CarbonMaterialBridge` (or keep v1 behavior
     via the bridge's `carbonTheme()`).
   - Pure Carbon users: `MaterialApp` → `CarbonApp`.
